...
class TcpInterface(Interface):
    ...
    def _transmit_receive(self, outbound_frames, response_lengths, timeout):
        ...
        for frame in frames:
            ...
            # Check if this is a Poll command (FrameFormat.WORD_DATA with POLL command)
            if message == b'\x05\x00':  # POLL command word
                # Poll command - check poll response queue
                if self.poll_response_queue.empty():
                    responses.append(_decode_frame(b'\x00\x00'))
                else:
                    responses.append(_decode_frame(self.poll_response_queue.get()))
            # Check if this is a PollAck command (FrameFormat.WORD_DATA with POLL_ACK command)
            elif message == b'\x45\x00':  # POLL_ACK command word (0x45)
